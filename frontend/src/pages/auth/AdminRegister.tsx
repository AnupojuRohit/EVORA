...
import { User, Mail, Lock, Eye, EyeOff, Shield, ArrowLeft, Loader2 } from 'lucide-react';
...
const AdminRegister = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } catch (error) { console.error('Admin registration failed:', error); } finally { setLoading(false); }
  };
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="backdrop-blur-xl bg-black/40 border border-white/10 rounded-2xl p-8 shadow-[0_0_50px_-10px_rgba(0,0,0,0.5)]">
          <div className="flex flex-col items-center mb-8 text-center">
            ...
            <h1 className="text-2xl font-bold tracking-tight">Register as Host</h1>
            <p className="text-gray-400 text-sm mt-2">Create your admin account to manage stations</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative group">
                ...
                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-12 pr-4 placeholder:text-gray-600 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-all" placeholder="Admin Name" required />
              </div>
            </div>
            ...
            <p className="text-center text-sm text-gray-500 mt-6">Already have an admin account?{' '}<Link to="/admin/login" className="text-emerald-400 hover:text-emerald-300 font-medium transition-colors hover:underline underline-offset-4">Sign in</Link></p>
            <div className="pt-4 border-t border-white/10"><Link to="/register" className="block text-center text-sm text-gray-500 hover:text-white transition-colors">← Register as User instead</Link></div>
          </form>
        </div>
      </div>
    </div>
