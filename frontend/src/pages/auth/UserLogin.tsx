...
import { Mail, Lock, Eye, EyeOff, ArrowLeft, Loader2, CarFront } from 'lucide-react';
...
const UserLogin = () => {
  ...
  const handleGoogleLogin = async () => {
    try { await authAPI.googleAuth(); } catch (error) { console.error('Google login failed:', error); }
  };
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="backdrop-blur-xl bg-black/40 border border-white/10 rounded-2xl p-8 shadow-[0_0_50px_-10px_rgba(0,0,0,0.5)]">
          <div className="flex flex-col items-center mb-8 text-center">
            ...
            <p className="text-gray-400 text-sm mt-2">Sign in to your <span className="text-emerald-400 font-medium">EVORA</span> account</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-12 pr-4 placeholder:text-gray-600 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-all"
            ...
            <div>
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-12 pr-12 placeholder:text-gray-600 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-all"
                  placeholder="••••••••"
                  required
                />
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="flex items-center gap-2 cursor-pointer group">
                <input type="checkbox" className="w-4 h-4 rounded border-white/20 bg-white/5 text-emerald-500 focus:ring-emerald-500/50 focus:ring-offset-0" />
                <span className="text-gray-400 group-hover:text-gray-300 transition-colors">Remember me</span>
              </label>
              <a href="#" className="text-emerald-400 hover:text-emerald-300 font-medium transition-colors hover:underline underline-offset-4">Forgot password?</a>
            </div>
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-sm"><span className="px-4 bg-[#09090b] text-gray-500">Or continue with</span></div>
            </div>

            <button type="button" onClick={handleGoogleLogin} className="w-full bg-white/5 hover:bg-white/10 border border-white/10 text-white font-medium py-3 rounded-xl transition-all flex items-center justify-center gap-3 backdrop-blur-sm">
            ...
            <p className="text-center text-sm text-gray-500 mt-6">Don't have an account?{' '}<Link to="/register" className="text-emerald-400 hover:text-emerald-300 font-medium transition-colors hover:underline underline-offset-4">Sign up</Link></p>
          </form>
        </div>
      </div>
    </div>
