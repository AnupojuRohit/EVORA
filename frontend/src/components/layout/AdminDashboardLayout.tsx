...
const AdminDashboardLayout = ({ children, userName = "Admin" }: AdminDashboardLayoutProps) => {
  ...
  const notifications = useMemo(
    () => [
      { id: "n1", title: "New station added", time: "5m ago", unread: true },
      { id: "n2", title: "10 bookings today", time: "1h ago", unread: true },
      { id: "n3", title: "Revenue milestone reached", time: "2h ago", unread: false },
    ],
    []
  )
  ...
  return (
    <div className="flex min-h-screen bg-[#070b10] text-slate-100">
      ...
      <aside className="group fixed lg:static inset-y-0 left-0 z-40 w-16 lg:w-16 hover:w-64 lg:hover:w-64 transition-[width,box-shadow] duration-300
        bg-gradient-to-b from-[#0c1218] via-[#0c151f] to-[#071019] text-white shadow-[0_25px_70px_-40px_rgba(168,85,247,0.5)] overflow-hidden
        ...
        <div className="px-3 py-6 border-b border-white/10">
          <Logo
            ...
            markClassName="shadow-[0_12px_24px_-10px_rgba(168,85,247,0.7)]"
          />
        </div>
        ...
        <div className="px-3 py-3 border-b border-white/10">
          <div className="flex items-center justify-center group-hover:justify-start gap-2">
            ...
            <span className="text-xs text-purple-300 opacity-0 max-w-0 overflow-hidden group-hover:max-w-[10rem] group-hover:opacity-100 transition-all duration-300">
              Admin Panel
            </span>
          </div>
        </div>
        ...
        <div className="mt-auto p-3 border-t border-white/10">
          <div className="flex items-center gap-3 mb-4 justify-center group-hover:justify-start transition-all">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-purple-400/30 to-pink-400/30 flex items-center justify-center text-white shadow-[0_0_16px_rgba(168,85,247,0.25)]">
              {name[0]}
            </div>
        ...
        <header className="sticky top-0 z-30 px-6 py-4 border-b border-white/10 flex items-center gap-4 bg-[#0a1016]/80 backdrop-blur-2xl">
          ...
          <div className="ml-auto flex items-center gap-3">
            <div className="relative hidden md:block">
              <div className="group flex items-center gap-2 h-10 w-72 rounded-full bg-white/5 border border-white/10 px-4 text-sm text-slate-200 focus-within:border-purple-300/60 focus-within:ring-2 focus-within:ring-purple-300/20 transition">
            ...
            <div className="relative">
              <button
                className="h-10 w-10 rounded-full border border-white/10 bg-white/5 hover:bg-white/10 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-300/40"
              ...
              >
                ...
                {unreadCount > 0 && (
                  <span className="absolute -right-0.5 -top-0.5 h-4 min-w-[16px] rounded-full bg-purple-400 text-[10px] text-slate-900 font-semibold flex items-center justify-center px-1 shadow-[0_0_12px_rgba(168,85,247,0.7)]">
                    {unreadCount}
                  </span>
                )}
              </button>
                      ...
                      {notifications.map(note => (
                        <div
                          ...
                          className={`rounded-xl border px-3 py-2 text-xs ${
                            note.unread
                              ? "border-purple-400/30 bg-purple-400/10 text-purple-50"
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 h-10 rounded-full border border-white/10 bg-white/5 px-2 pr-3 hover:bg-white/10 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-300/40"
                aria-label="Open profile menu"
                aria-expanded={profileOpen}
              >
                <div className="h-8 w-8 rounded-full bg-gradient-to-br from-purple-300/60 to-pink-300/60 text-slate-900 flex items-center justify-center text-xs font-semibold">
                  {name[0]}
                </div>
              ...
              <AnimatePresence>
                {profileOpen && (
                  ...
                  >
                    <button className="w-full flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-slate-200 hover:bg-white/5 transition">
